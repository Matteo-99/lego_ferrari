...
Modified solution starting from the Atsushi Sakai(@Atsushi_twi) code.

The planner calculate the possible paths from the StartingPose to the GoalPose, then  it 
choose the best one in terms of length. Finally the best path is represented in a figure and 
the function returns a vector containing all the points of our path where the robot change direction
(forward or backward), plus obviosly the final goal.
"""
...
def plot_arrow(ax, Pose, length=1.0, width=0.5, fc="r", ec="k"):
    x = Pose.x
    y = Pose.y
    yaw = Pose.theta
    if isinstance(x, list):
        ...
        ax.arrow(x, y, length * math.cos(yaw), length * math.sin(yaw), fc=fc,
                  ec=ec, head_width=width, head_length=width, length_includes_head = True)
        ax.plot(x, y)
...
def reeds_shepp_path_planning(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=0.2):
    ...
    if not paths:
        return None, None, None, None, None, None  # could not generate any path
...
def planner(StartPose, FinalPose, wheelbase, curvature, step_size, ax):
    ...
    if not xs:
        ...
        if show_animation:  # pragma: no cover
            ax.cla()
            ax.plot(xs, ys, label="final course " + str(modes))

            # plotting
            plot_arrow(ax, StartPose, wheelbase, 0.5*wheelbase, fc="r")
            plot_arrow(ax, FinalPose, wheelbase, 0.5*wheelbase, fc="g")

            ax.legend()
            ax.grid(True)
            ax.axis("equal")
    
        goal_vec = find_goals(xs,ys,yaws,directions)
        return goal_vec
